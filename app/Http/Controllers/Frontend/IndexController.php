...
class IndexController extends Controller
{
    ...
    public function SaveNotifyUSer($email){
      $validator = Validator::make(['email'=>$email],[
			'email' => 'required|email',
		]);
      if($validator->fails())
		{
         return response()->json(['success'=>false,'data' => "Must be an email"]);
		}
      $user_exist = DB::table('anticipating_users')->where('email',$email)->exists();
      if($user_exist){
         return response()->json(['success'=>false,'data' => "User Email Already Exist"]);
      }
      $insert = DB::table('anticipating_users')->insert([
         'email' => $email,
         'created_at' => now()
      ]);
      if(!$insert){
         return response()->json(['success'=>false,'data' => "Error inserting"]);
      }
      return response()->json(['success'=>true,'data' => "You will be notified when we launch. Thanks"]);
    }
